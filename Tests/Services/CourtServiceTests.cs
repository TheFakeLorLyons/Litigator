...
public class CourtServiceTests : IDisposable
{
    ...
    public CourtServiceTests()
    {
        ...
        _createCourtFaker = new Faker<CreateCourtDTO>()
            ...
            .RuleFor(c => c.Address, (f, c) => new AddressDTO  // Note: using (f, c) to access the current object
            {
                Line1 = f.Address.StreetAddress(),
                City = f.Address.City(),
                County = c.County, // Use the County that was already set at the root level
                State = c.State,
